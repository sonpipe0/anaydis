...
public class QuickSortMedianThree extends AbstractSorter{
    private int M = 16;
    ...
    private <T> void quickSort(List<T> ls, int lo,int hi,Comparator<T> cmp){
        if (hi-lo <= M) return;
        exch(ls, (lo+hi)/2, hi-1);
        compExch(ls, lo, hi-1,cmp);
        compExch(ls, lo, hi,cmp);
        compExch(ls, hi-1, hi,cmp);
        int i = partition(ls, lo+1, hi-1,cmp);
